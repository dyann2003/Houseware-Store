...
<html>
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-hover text-nowrap">
                                    ...
                                    <tbody>
                                        <c:forEach items="${listTop5Customer}" var="c">
                                            <tr class="invoice">
                                                <td class="text_page">${c.user.fullName}</td>
                                                <td class="text_page">${c.user.userName}</td>
                                                <td class="text_page">${c.user.phone}</td> 
                                                <td class="text_page">${c.date}</td>
                                                <td class="text_page format">
                                                    <input type="hidden" id="amount" value="${c.total}">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            let productContainers = document.querySelectorAll('.invoice');

            // Loop through each product container
            productContainers.forEach(function (productContainer) {
                // Get the price element within this product container
                let priceElement = productContainer.querySelector('.format'); // Change the class name to text_page

                // Get the amount from the hidden input inside this price element
                let amount = parseFloat(priceElement.querySelector("input[type='hidden']").value);
